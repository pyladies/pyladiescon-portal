...
class TestSignupView:
    def test_error_styling_on_invalid_signup(self, client):
        ...
        assertContains(response, 'is-invalid', status_code=200)  # Bootstrap invalid class
        assertContains(response, 'invalid-feedback', status_code=200)  # Error message class
        
        # Verify specific field errors
        assertContains(response, 'This field is required', status_code=200)  # username
        assertContains(response, 'Enter a valid email address', status_code=200)  # email

    def test_widget_tweaks_loaded(self, client):
        response = client.get(reverse('account_signup'))
        assertContains(response, 'form-control', status_code=200)  # Verify Bootstrap styling
